...
def subsequent_mask(size):
    ...
    subsequent_mask_matrtix = np.triu(np.ones(attn_shape), k=1).astype('uint8')
    return torch.from_numpy(subsequent_mask_matrtix) == 0
...
class PositionalEncoding(nn.Module):
    """Implement the PositionEncoding function."""
...
def make_model(src_vocab_size, tgt_vocab_size, N=6,
    ...
    model = EncoderDecoder(
        ...
        Decoder(DecoderLayer(d_model, attn, attn,
                             ff, dropout), N),
        nn.Sequential(Embeddings(d_model, src_vocab_size), position),
        nn.Sequential(Embeddings(d_model, tgt_vocab_size), position),
